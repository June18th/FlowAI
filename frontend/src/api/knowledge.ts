...
export const createKnowledgeBase = (data: {
  ...
  chunkSize?: number;
  chunkOverlap?: number;
}): Promise<ApiResult<KnowledgeBase>> => api.post('/api/v1/knowledge-bases', data);

export const getKnowledgeBase = (id: number): Promise<ApiResult<KnowledgeBase>> =>
  api.get(`/api/v1/knowledge-bases/${id}`);
...
export const importKnowledgeText = (
  ...
  api.post(`/api/v1/knowledge-bases/${id}/documents/text`, data);
...
export const uploadKnowledgeTextFile = (
  ...
  return api.post(`/api/v1/knowledge-bases/${id}/documents/upload`, formData, {
...
export const previewKnowledgeChunks = (
  ...
  api.post(`/api/v1/knowledge-bases/${id}/documents/${documentId}/preview-chunks`, data || {});

export const indexKnowledgeDocument = (
  id: number,
  documentId: number
): Promise<ApiResult<KnowledgeIndexTask>> =>
  api.post(`/api/v1/knowledge-bases/${id}/documents/${documentId}/index`);
...
export const searchKnowledgeBase = (
  ...
  api.get(`/api/v1/knowledge-bases/${id}/chunks`, { params: data });
